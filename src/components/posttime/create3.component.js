...
export default function PostCreate() {
  ...
  const createPostTime = (e) => {
    ...
    for (const i in state.details) {
      ...
      if (state.details[i].postTime[0].label == '00.00 - 02.00') {
        let postsPerson = [];

        if (state.details[i].postTime[0].value == true) {
          let norPerson = [{ name: 'Deneme Asker' }];
          for (let yy = 0; yy < Number(state.details[i].person); yy++) {
            let params = [
              { nowDate: NowDate },
              { time: state.details[i].postTime[0].label },
              { norPerson: norPerson },
            ];
