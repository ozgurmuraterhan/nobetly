...
export default function  CustomersList() {
  ...
  const columns = [
     ...
     
    {
      ...
      render: (rowData) => (
        <div>
          <Link to={`/post/edit/${rowData._id}`}><Edit /></Link>
        </div>
      ),
    },
  ]
  ...
  const getGroupNameStatistic= () => {
    ...
    axios.get('http://localhost:5000/post/statistic')
      .then((response) => {
        if (response.data.length > 0) {
          const details_label = [];
          const details_value = [];
               ...
               <MaterialTable
                 ...
                 editable={{
                   onRowAdd: (newData) => new Promise((resolve, reject) => {
                     axios.post('http://localhost:5000/postgroups/add', { name: newData.name })
                       .then((response) => {
                   ...
                   onRowUpdate: (newData, oldData) => new Promise((resolve, reject) => {
                     axios.post(`http://localhost:5000/postgroups/${newData._id}`, { name: newData.name })
                       .then((response) => {
                   ...
                   onRowDelete: (oldData) => new Promise((resolve, reject) => {
                     axios.delete(`http://localhost:5000/postgroups/${oldData._id}`)
                       .then((response) => {
                         const index = customergroups.indexOf(oldData);
                     ...
                     Toolbar: (props) => (
                       <div>
                        <Typography component="h5" variant="h6" color="inherit" noWrap className="typography">
                          {t('Nöbet Yeri Listesi')}
                        </Typography>
                        <Link to="/PostCreate" className="addButtonPlace">
             ...
             <Grid item container md={3} className="panelGridRelative">
               <Card className="listViewPaper" style={{ padding: '10px' }}>
                 <Typography component="h1" variant="h6" color="inherit" noWrap style={{ width: '100%', paddingLeft: '10px' }} className="typography">
                   {t('Nöbet Yeri Kategorisi')}
                   <Tooltip title={t('Nöbet Yeri Kategorisini Düzenle')}>
