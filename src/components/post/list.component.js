...
export default function CustomersList() {
  ...
  const getCustomersGroupData = () => {
    axios.get('http://localhost:5000/postgroups').then((response) => {
      if (response.data.length > 0) {
  ...
  const getGroupNameStatistic = () => {
    ...
    axios.get('http://localhost:5000/post/statistic').then((response) => {
      if (response.data.length > 0) {
        const details_label = [];
        const details_value = [];
              ...
              editable={{
                onRowAdd: (newData) =>
                  new Promise((resolve, reject) => {
                    axios
                      .post('http://localhost:5000/postgroups/add', {
                        name: newData.name,
                      })
                      .then((response) => {
                ...
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    axios
                      .post(`http://localhost:5000/postgroups/${newData._id}`, {
                        name: newData.name,
                      })
                      .then((response) => {
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    axios
                      .delete(`http://localhost:5000/postgroups/${oldData._id}`)
                      .then((response) => {
                        const index = customergroups.indexOf(oldData);
