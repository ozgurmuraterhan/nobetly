...
export default function SoldierEdit(props) {
  ...
  const deleteData = (id) => {
    axios.delete(`http://localhost:5000/soldier/${id}`).then((res) => {
      if (res.data.variant == 'error') {
        enqueueSnackbar(t('Asker Silinemedi') + res.data.messagge, {
          variant: res.data.variant,
        });
      } else {
        enqueueSnackbar(t('Asker Silindi'), { variant: res.data.variant });
  ...
  function getPost() {
    axios
      .get(`http://localhost:5000/soldier/${props.match.params.id}`)
      .then((res) => {
        ...
        seTstate({
          ...
          thinks: state.thinks,
          risk: res.data.risk,
          over_date: res.data.over_date,
          selectedDefaultState: [
            {
              label: res.data.defaultAddress_state_id,
              value: res.data.defaultAddress_state_id,
            },
          ],
          selected1Town: res.data.defaultAddress_town,
          selected1Zipcode: res.data.defaultAddress_zipcode,
          selected1Address: res.data.defaultAddress_address,
  ...
  const onSubmit = (e) => {
    ...
    axios
      .post(`http://localhost:5000/soldier/${props.match.params.id}`, Soldier)
      .then((res) => {
        if (res.data.variant == 'error') {
          enqueueSnackbar(t('Asker Güncellenemedi') + res.data.messagge, {
            variant: res.data.variant,
          });
        } else {
          enqueueSnackbar(t('Asker Güncellendi'), {
            variant: res.data.variant,
          });
          // navigate
          history.push('/Soldierlist');
